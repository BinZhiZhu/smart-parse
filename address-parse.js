...
const provinceKey = ['特别行政区', '古自治区', '新疆维吾尔自治区', '壮族自治区', '回族自治区', '自治区', '省省直辖', '省', '市'];
const cityKey = ['布依族苗族自治州', '苗族侗族自治州', '克孜勒苏柯尔克孜自治州','自治州', '州', '市', '县'];

/**
 * todo 这里改成了weui的地址库处理
 * @param list
 * @param init
 * @returns {boolean}
 */
function initAddress(list, init) {
    ...
    defaultData.forEach(province => {
        if (province.sub) {
            province.sub.forEach(city => {
                ...
                if(city.sub){
                    city.sub.forEach(area=>{
                        if (area.name !== '其他') {
                            if (!mArea[area.name]) {
                                mArea[area.name] = [];
                            }
                            mArea[area.name].push({
                                p: province.name,
                                c: city.name
                            })
                        }
 ...
 * @returns {boolean}
 */
function matchAddress(list, init) {
    if (!init && defaultData.length) {
        return true;
    }
    defaultData = list;
...
function parse(address) {
    ...
    console.log('正向解析处理，结果:',detail)

    // if(provinceKey.indexOf(detail.province) ){
    //     detail.province = detail.province;
    //     if(cityKey.indexOf(detail.addr)){
    //         for (let c in cityKey){
    //             if (detail.addr.indexOf(cityKey[c]) === 0) {
    //                 detail.city = cityKey[c];
    //                 let carea= detail.addr.substr(cityKey[c].length);
    //                 let cindex = carea.indexOf('市');
    //                 if(cindex >-1){
    //                     detail.addr = carea.substr(cindex+1);
    //                     detail.area = carea.slice(0,cindex+1);
    //                 }
    //             }
    //         }
    //     }
    ...
    parse.street = detail.addr;
    parse.result = detail.result === undefined || detail.result === '' ? '' : detail.result;
    return parse;
}
...
function detail_parse_forward(address) {
    ...
    const provinceKey = ['特别行政区', '古自治区', '新疆维吾尔自治区', '壮族自治区', '回族自治区', '自治区', '省省直辖','省', '市'];
    const cityKey = ['布依族苗族自治州', '苗族侗族自治州', '自治州','克孜勒苏柯尔克孜自治州', '州', '市', '县'];

    for (let i in defaultData) {
        const province = defaultData[i];
        // console.log('sub',province)
        ...
        if (index > -1) {
            ...
            for (let j in province.city) {
                ...
                if (index > -1 && index < 3) {
                    ...
                    for (let k in cityKey) {
                        // todo 这里是兼容部分市辖区
                        if (address.indexOf(cityKey[k]) === 0 && address.indexOf('市辖区') <-1) {
            ...
            //这是是匹配 xxx人+电话 没有填写省市区直接走到这里判断处理了
            console.log('address',address)
            let realname = address;
            parse.name = realname;
        }
    }
    return parse;
 ...
 * 有地区就能大概返回地址了
 * @param address
 * @param ignoreArea 是否忽市区 因为地址中含有区容易导致匹配错误 例：山东省蓬莱市黄海花园东区西门宝威学堂 曲荣声收15753572456
...
function detail_parse(address, {ignoreArea = false} = {}) {
    ...
    if(provinceKey.indexOf(address) < 0 && cityKey.indexOf(address) < 0){
        console.log('--匹配姓名--')
        parse.name = address;
        return parse;
    }
